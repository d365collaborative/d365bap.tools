Describe "Invoke-BapInstallAzCopy Unit Tests" -Tag "Unit" {
	BeforeAll {
	...
	Describe "Ensuring unchanged command signature" {
		It "should have the expected parameter sets" {
			(Get-Command Invoke-BapInstallAzCopy).ParameterSets.Name | Should -Be '__AllParameterSets'
		}
		
		It 'Should have the expected parameter Url' {
			$parameter = (Get-Command Invoke-BapInstallAzCopy).Parameters['Url']
		...
		It 'Should have the expected parameter Path' {
			$parameter = (Get-Command Invoke-BapInstallAzCopy).Parameters['Path']
		...
		It 'Should have the expected parameter ProgressAction' {
			$parameter = (Get-Command Invoke-BapInstallAzCopy).Parameters['ProgressAction']
	...
	Describe "Testing parameterset __AllParameterSets" {
		<#
		__AllParameterSets -
		__AllParameterSets -Url -Path -ProgressAction
		#>
	}
