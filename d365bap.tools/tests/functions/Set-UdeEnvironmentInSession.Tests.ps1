Describe "Set-UdeEnvironmentInSession Unit Tests" -Tag "Unit" {
	BeforeAll {
	...
	Describe "Ensuring unchanged command signature" {
		It "should have the expected parameter sets" {
			(Get-Command Set-UdeEnvironmentInSession).ParameterSets.Name | Should -Be '__AllParameterSets'
		}
		
		It 'Should have the expected parameter EnvironmentId' {
			$parameter = (Get-Command Set-UdeEnvironmentInSession).Parameters['EnvironmentId']
		...
		It 'Should have the expected parameter ProgressAction' {
			$parameter = (Get-Command Set-UdeEnvironmentInSession).Parameters['ProgressAction']
