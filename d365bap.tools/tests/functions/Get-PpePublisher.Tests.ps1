Describe "Get-PpePublisher Unit Tests" -Tag "Unit" {
	BeforeAll {
	...
	Describe "Ensuring unchanged command signature" {
		It "should have the expected parameter sets" {
			(Get-Command Get-PpePublisher).ParameterSets.Name | Should -Be '__AllParameterSets'
		}
		
		It 'Should have the expected parameter EnvironmentId' {
			$parameter = (Get-Command Get-PpePublisher).Parameters['EnvironmentId']
		...
		It 'Should have the expected parameter Name' {
			$parameter = (Get-Command Get-PpePublisher).Parameters['Name']
		...
		It 'Should have the expected parameter AsExcelOutput' {
			$parameter = (Get-Command Get-PpePublisher).Parameters['AsExcelOutput']
		...
		It 'Should have the expected parameter AsHashTable' {
			$parameter = (Get-Command Get-PpePublisher).Parameters['AsHashTable']
		...
		It 'Should have the expected parameter ProgressAction' {
			$parameter = (Get-Command Get-PpePublisher).Parameters['ProgressAction']
	...
	Describe "Testing parameterset __AllParameterSets" {
		...
		__AllParameterSets -EnvironmentId -Name -AsExcelOutput -AsHashTable -ProgressAction
		#>
	}
