Describe "Get-FscmDmfEntity Unit Tests" -Tag "Unit" {
	BeforeAll {
	...
	Describe "Ensuring unchanged command signature" {
		It "should have the expected parameter sets" {
			(Get-Command Get-FscmDmfEntity).ParameterSets.Name | Should -Be '__AllParameterSets'
		}
		
		It 'Should have the expected parameter EnvironmentId' {
			$parameter = (Get-Command Get-FscmDmfEntity).Parameters['EnvironmentId']
		...
		It 'Should have the expected parameter Name' {
			$parameter = (Get-Command Get-FscmDmfEntity).Parameters['Name']
		...
		It 'Should have the expected parameter AsExcelOutput' {
			$parameter = (Get-Command Get-FscmDmfEntity).Parameters['AsExcelOutput']
		...
		It 'Should have the expected parameter ProgressAction' {
			$parameter = (Get-Command Get-FscmDmfEntity).Parameters['ProgressAction']
