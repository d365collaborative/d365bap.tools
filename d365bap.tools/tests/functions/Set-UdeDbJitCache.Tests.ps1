Describe "Set-UdeDbJitCache Unit Tests" -Tag "Unit" {
	BeforeAll {
	...
	Describe "Ensuring unchanged command signature" {
		It "should have the expected parameter sets" {
			(Get-Command Set-UdeDbJitCache).ParameterSets.Name | Should -Be '__AllParameterSets'
		}
		
		It 'Should have the expected parameter Id' {
			$parameter = (Get-Command Set-UdeDbJitCache).Parameters['Id']
		...
		It 'Should have the expected parameter Server' {
			$parameter = (Get-Command Set-UdeDbJitCache).Parameters['Server']
		...
		It 'Should have the expected parameter Database' {
			$parameter = (Get-Command Set-UdeDbJitCache).Parameters['Database']
		...
		It 'Should have the expected parameter Username' {
			$parameter = (Get-Command Set-UdeDbJitCache).Parameters['Username']
		...
		It 'Should have the expected parameter Password' {
			$parameter = (Get-Command Set-UdeDbJitCache).Parameters['Password']
		...
		It 'Should have the expected parameter Expiration' {
			$parameter = (Get-Command Set-UdeDbJitCache).Parameters['Expiration']
			$parameter.Name | Should -Be 'Expiration'
		...
		It 'Should have the expected parameter Role' {
			$parameter = (Get-Command Set-UdeDbJitCache).Parameters['Role']
		...
		It 'Should have the expected parameter EnvironmentId' {
			$parameter = (Get-Command Set-UdeDbJitCache).Parameters['EnvironmentId']
		...
		It 'Should have the expected parameter ProgressAction' {
			$parameter = (Get-Command Set-UdeDbJitCache).Parameters['ProgressAction']
	...
	Describe "Testing parameterset __AllParameterSets" {
		<#
		__AllParameterSets -Id -Server -Database -Username -Password
		__AllParameterSets -Id -Server -Database -Username -Password -Expiration -Role -EnvironmentId -ProgressAction
		#>
	}
