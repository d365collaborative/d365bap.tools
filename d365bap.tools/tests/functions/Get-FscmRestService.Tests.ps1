Describe "Get-FscmRestService Unit Tests" -Tag "Unit" {
	BeforeAll {
	...
	Describe "Ensuring unchanged command signature" {
		It "should have the expected parameter sets" {
			(Get-Command Get-FscmRestService).ParameterSets.Name | Should -Be '__AllParameterSets'
		}
		
		It 'Should have the expected parameter EnvironmentId' {
			$parameter = (Get-Command Get-FscmRestService).Parameters['EnvironmentId']
		...
		It 'Should have the expected parameter Name' {
			$parameter = (Get-Command Get-FscmRestService).Parameters['Name']
		...
		It 'Should have the expected parameter TraverseTo' {
			$parameter = (Get-Command Get-FscmRestService).Parameters['TraverseTo']
			$parameter.Name | Should -Be 'TraverseTo'
			$parameter.ParameterType.ToString() | Should -Be System.String
			$parameter.IsDynamic | Should -Be $False
			$parameter.ParameterSets.Keys | Should -Be '__AllParameterSets'
			$parameter.ParameterSets.Keys | Should -Contain '__AllParameterSets'
		...
		It 'Should have the expected parameter AsExcelOutput' {
			$parameter = (Get-Command Get-FscmRestService).Parameters['AsExcelOutput']
		...
		It 'Should have the expected parameter ProgressAction' {
			$parameter = (Get-Command Get-FscmRestService).Parameters['ProgressAction']
	...
	Describe "Testing parameterset __AllParameterSets" {
		...
		__AllParameterSets -EnvironmentId -Name -TraverseTo -AsExcelOutput -ProgressAction
		#>
	}
