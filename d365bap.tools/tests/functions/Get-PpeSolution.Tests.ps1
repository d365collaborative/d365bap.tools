Describe "Get-PpeSolution Unit Tests" -Tag "Unit" {
	BeforeAll {
	...
	Describe "Ensuring unchanged command signature" {
		It "should have the expected parameter sets" {
			(Get-Command Get-PpeSolution).ParameterSets.Name | Should -Be '__AllParameterSets'
		}
		
		It 'Should have the expected parameter EnvironmentId' {
			$parameter = (Get-Command Get-PpeSolution).Parameters['EnvironmentId']
		...
		It 'Should have the expected parameter Name' {
			$parameter = (Get-Command Get-PpeSolution).Parameters['Name']
		...
		It 'Should have the expected parameter AsExcelOutput' {
			$parameter = (Get-Command Get-PpeSolution).Parameters['AsExcelOutput']
		...
		It 'Should have the expected parameter ProgressAction' {
			$parameter = (Get-Command Get-PpeSolution).Parameters['ProgressAction']
