Describe "Set-BapAzCopyPath Unit Tests" -Tag "Unit" {
	BeforeAll {
	...
	Describe "Ensuring unchanged command signature" {
		It "should have the expected parameter sets" {
			(Get-Command Set-BapAzCopyPath).ParameterSets.Name | Should -Be '__AllParameterSets'
		}
		
		It 'Should have the expected parameter Path' {
			$parameter = (Get-Command Set-BapAzCopyPath).Parameters['Path']
		...
		It 'Should have the expected parameter ProgressAction' {
			$parameter = (Get-Command Set-BapAzCopyPath).Parameters['ProgressAction']
	...
	Describe "Testing parameterset __AllParameterSets" {
		...
		__AllParameterSets -Path -ProgressAction
		#>
	}
