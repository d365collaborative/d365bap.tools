Describe "Get-BapEnvironmentFnOAppUpdate Unit Tests" -Tag "Unit" {
	BeforeAll {
	...
	Describe "Ensuring unchanged command signature" {
		It "should have the expected parameter sets" {
			(Get-Command Get-BapEnvironmentFnOAppUpdate).ParameterSets.Name | Should -Be 'Default', 'Lowest', 'Highest'
		}
		
		It 'Should have the expected parameter EnvironmentId' {
			$parameter = (Get-Command Get-BapEnvironmentFnOAppUpdate).Parameters['EnvironmentId']
		...
		It 'Should have the expected parameter Oldest' {
			$parameter = (Get-Command Get-BapEnvironmentFnOAppUpdate).Parameters['Oldest']
		...
		It 'Should have the expected parameter Latest' {
			$parameter = (Get-Command Get-BapEnvironmentFnOAppUpdate).Parameters['Latest']
