Describe "Get-PpeOdata Unit Tests" -Tag "Unit" {
	BeforeAll {
	...
	Describe "Ensuring unchanged command signature" {
		It "should have the expected parameter sets" {
			(Get-Command Get-PpeOdata).ParameterSets.Name | Should -Be 'Default', 'NextLink'
		}
		
		It 'Should have the expected parameter EnvironmentId' {
			$parameter = (Get-Command Get-PpeOdata).Parameters['EnvironmentId']
		...
		It 'Should have the expected parameter Entity' {
			$parameter = (Get-Command Get-PpeOdata).Parameters['Entity']
		...
		It 'Should have the expected parameter ODataQuery' {
			$parameter = (Get-Command Get-PpeOdata).Parameters['ODataQuery']
		...
		It 'Should have the expected parameter TraverseNextLink' {
			$parameter = (Get-Command Get-PpeOdata).Parameters['TraverseNextLink']
		...
		It 'Should have the expected parameter ThrottleSeed' {
			$parameter = (Get-Command Get-PpeOdata).Parameters['ThrottleSeed']
		...
		It 'Should have the expected parameter AsExcelOutput' {
			$parameter = (Get-Command Get-PpeOdata).Parameters['AsExcelOutput']
		...
		It 'Should have the expected parameter ProgressAction' {
			$parameter = (Get-Command Get-PpeOdata).Parameters['ProgressAction']
	...
	Describe "Testing parameterset Default" {
		...
		Default -EnvironmentId -Entity -ODataQuery -AsExcelOutput -ProgressAction
		#>
	}
 	Describe "Testing parameterset NextLink" {
		<#
		NextLink -EnvironmentId -Entity -TraverseNextLink
		NextLink -EnvironmentId -Entity -ODataQuery -TraverseNextLink -ThrottleSeed -AsExcelOutput -ProgressAction
		#>
	}
