Describe "Remove-UnifiedEnvironmentPackage Unit Tests" -Tag "Unit" {
	BeforeAll {
	...
	Describe "Ensuring unchanged command signature" {
		It "should have the expected parameter sets" {
			(Get-Command Remove-UnifiedEnvironmentPackage).ParameterSets.Name | Should -Be '__AllParameterSets'
		}
		
		It 'Should have the expected parameter EnvironmentId' {
			$parameter = (Get-Command Remove-UnifiedEnvironmentPackage).Parameters['EnvironmentId']
		...
		It 'Should have the expected parameter PackageId' {
			$parameter = (Get-Command Remove-UnifiedEnvironmentPackage).Parameters['PackageId']
			$parameter.Name | Should -Be 'PackageId'
			$parameter.ParameterType.ToString() | Should -Be System.String
			$parameter.IsDynamic | Should -Be $False
			$parameter.ParameterSets.Keys | Should -Be '__AllParameterSets'
			$parameter.ParameterSets.Keys | Should -Contain '__AllParameterSets'
		...
		It 'Should have the expected parameter Name' {
			$parameter = (Get-Command Remove-UnifiedEnvironmentPackage).Parameters['Name']
		...
		It 'Should have the expected parameter Force' {
			$parameter = (Get-Command Remove-UnifiedEnvironmentPackage).Parameters['Force']
		...
		It 'Should have the expected parameter ProgressAction' {
			$parameter = (Get-Command Remove-UnifiedEnvironmentPackage).Parameters['ProgressAction']
	...
	Describe "Testing parameterset __AllParameterSets" {
		...
		__AllParameterSets -EnvironmentId -PackageId -Name -Force -ProgressAction
		#>
	}
