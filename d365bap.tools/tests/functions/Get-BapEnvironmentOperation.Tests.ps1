Describe "Get-BapEnvironmentOperation Unit Tests" -Tag "Unit" {
	BeforeAll {
	...
	Describe "Ensuring unchanged command signature" {
		It "should have the expected parameter sets" {
			(Get-Command Get-BapEnvironmentOperation).ParameterSets.Name | Should -Be '__AllParameterSets'
		}
		
		It 'Should have the expected parameter EnvironmentId' {
			$parameter = (Get-Command Get-BapEnvironmentOperation).Parameters['EnvironmentId']
		...
		It 'Should have the expected parameter LatestOnly' {
			$parameter = (Get-Command Get-BapEnvironmentOperation).Parameters['LatestOnly']
		...
		It 'Should have the expected parameter AsExcelOutput' {
			$parameter = (Get-Command Get-BapEnvironmentOperation).Parameters['AsExcelOutput']
		...
		It 'Should have the expected parameter ProgressAction' {
			$parameter = (Get-Command Get-BapEnvironmentOperation).Parameters['ProgressAction']
