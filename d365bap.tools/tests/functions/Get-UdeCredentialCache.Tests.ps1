Describe "Get-UdeCredentialCache Unit Tests" -Tag "Unit" {
	BeforeAll {
	...
	Describe "Ensuring unchanged command signature" {
		It "should have the expected parameter sets" {
			(Get-Command Get-UdeCredentialCache).ParameterSets.Name | Should -Be '__AllParameterSets'
		}
		
		It 'Should have the expected parameter Path' {
			$parameter = (Get-Command Get-UdeCredentialCache).Parameters['Path']
		...
		It 'Should have the expected parameter AsExcelOutput' {
			$parameter = (Get-Command Get-UdeCredentialCache).Parameters['AsExcelOutput']
		...
		It 'Should have the expected parameter ProgressAction' {
			$parameter = (Get-Command Get-UdeCredentialCache).Parameters['ProgressAction']
	...
	Describe "Testing parameterset __AllParameterSets" {
		<#
		__AllParameterSets -
		__AllParameterSets -Path -AsExcelOutput -ProgressAction
		#>
	}
