Describe "Remove-FscmUser Unit Tests" -Tag "Unit" {
	BeforeAll {
	...
	Describe "Ensuring unchanged command signature" {
		It "should have the expected parameter sets" {
			(Get-Command Remove-FscmUser).ParameterSets.Name | Should -Be '__AllParameterSets'
		}
		
		It 'Should have the expected parameter EnvironmentId' {
			$parameter = (Get-Command Remove-FscmUser).Parameters['EnvironmentId']
		...
		It 'Should have the expected parameter User' {
			$parameter = (Get-Command Remove-FscmUser).Parameters['User']
		...
		It 'Should have the expected parameter Role' {
			$parameter = (Get-Command Remove-FscmUser).Parameters['Role']
		...
		It 'Should have the expected parameter ProgressAction' {
			$parameter = (Get-Command Remove-FscmUser).Parameters['ProgressAction']
