Describe "Get-PpacD365App Unit Tests" -Tag "Unit" {
	BeforeAll {
	...
	Describe "Ensuring unchanged command signature" {
		It "should have the expected parameter sets" {
			(Get-Command Get-PpacD365App).ParameterSets.Name | Should -Be '__AllParameterSets'
		}
		
		It 'Should have the expected parameter EnvironmentId' {
			$parameter = (Get-Command Get-PpacD365App).Parameters['EnvironmentId']
		...
		It 'Should have the expected parameter Name' {
			$parameter = (Get-Command Get-PpacD365App).Parameters['Name']
		...
		It 'Should have the expected parameter AsExcelOutput' {
			$parameter = (Get-Command Get-PpacD365App).Parameters['AsExcelOutput']
		...
		It 'Should have the expected parameter ProgressAction' {
			$parameter = (Get-Command Get-PpacD365App).Parameters['ProgressAction']
