Describe "Invoke-BapEnvironmentFnOAppUpdate Unit Tests" -Tag "Unit" {
	BeforeAll {
	...
	Describe "Ensuring unchanged command signature" {
		It "should have the expected parameter sets" {
			(Get-Command Invoke-BapEnvironmentFnOAppUpdate).ParameterSets.Name | Should -Be '__AllParameterSets'
		}
		
		It 'Should have the expected parameter EnvironmentId' {
			$parameter = (Get-Command Invoke-BapEnvironmentFnOAppUpdate).Parameters['EnvironmentId']
