Describe "Add-FscmSecurityRoleMember Unit Tests" -Tag "Unit" {
	BeforeAll {
	...
	Describe "Ensuring unchanged command signature" {
		It "should have the expected parameter sets" {
			(Get-Command Add-FscmSecurityRoleMember).ParameterSets.Name | Should -Be '__AllParameterSets'
		}
		
		It 'Should have the expected parameter EnvironmentId' {
			$parameter = (Get-Command Add-FscmSecurityRoleMember).Parameters['EnvironmentId']
		...
		It 'Should have the expected parameter User' {
			$parameter = (Get-Command Add-FscmSecurityRoleMember).Parameters['User']
		...
		It 'Should have the expected parameter Role' {
			$parameter = (Get-Command Add-FscmSecurityRoleMember).Parameters['Role']
