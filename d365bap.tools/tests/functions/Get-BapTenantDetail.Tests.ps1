Describe "Get-BapTenantDetail Unit Tests" -Tag "Unit" {
	BeforeAll {
	...
	Describe "Ensuring unchanged command signature" {
		It "should have the expected parameter sets" {
			(Get-Command Get-BapTenantDetail).ParameterSets.Name | Should -Be '__AllParameterSets'
		}
		
		It 'Should have the expected parameter Id' {
			$parameter = (Get-Command Get-BapTenantDetail).Parameters['Id']
		...
		It 'Should have the expected parameter Upn' {
			$parameter = (Get-Command Get-BapTenantDetail).Parameters['Upn']
		...
		It 'Should have the expected parameter TenantId' {
			$parameter = (Get-Command Get-BapTenantDetail).Parameters['TenantId']
		...
		It 'Should have the expected parameter FriendlyName' {
			$parameter = (Get-Command Get-BapTenantDetail).Parameters['FriendlyName']
			$parameter.Name | Should -Be 'FriendlyName'
			$parameter.ParameterType.ToString() | Should -Be System.String
			$parameter.IsDynamic | Should -Be $False
			$parameter.ParameterSets.Keys | Should -Be '__AllParameterSets'
			$parameter.ParameterSets.Keys | Should -Contain '__AllParameterSets'
		...
		It 'Should have the expected parameter AsExcelOutput' {
			$parameter = (Get-Command Get-BapTenantDetail).Parameters['AsExcelOutput']
		...
		It 'Should have the expected parameter ProgressAction' {
			$parameter = (Get-Command Get-BapTenantDetail).Parameters['ProgressAction']
	...
	Describe "Testing parameterset __AllParameterSets" {
		<#
		__AllParameterSets -
		__AllParameterSets -Id -Upn -TenantId -FriendlyName -AsExcelOutput -ProgressAction
		#>
	}
