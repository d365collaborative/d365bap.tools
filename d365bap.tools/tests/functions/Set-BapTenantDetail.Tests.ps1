Describe "Set-BapTenantDetail Unit Tests" -Tag "Unit" {
	BeforeAll {
	...
	Describe "Ensuring unchanged command signature" {
		It "should have the expected parameter sets" {
			(Get-Command Set-BapTenantDetail).ParameterSets.Name | Should -Be '__AllParameterSets'
		}
		
		It 'Should have the expected parameter Id' {
			$parameter = (Get-Command Set-BapTenantDetail).Parameters['Id']
		...
		It 'Should have the expected parameter Upn' {
			$parameter = (Get-Command Set-BapTenantDetail).Parameters['Upn']
		...
		It 'Should have the expected parameter TenantId' {
			$parameter = (Get-Command Set-BapTenantDetail).Parameters['TenantId']
		...
		It 'Should have the expected parameter TenantName' {
			$parameter = (Get-Command Set-BapTenantDetail).Parameters['TenantName']
			$parameter.Name | Should -Be 'TenantName'
			$parameter.ParameterType.ToString() | Should -Be System.String
			$parameter.IsDynamic | Should -Be $False
			$parameter.ParameterSets.Keys | Should -Be '__AllParameterSets'
			$parameter.ParameterSets.Keys | Should -Contain '__AllParameterSets'
		...
		It 'Should have the expected parameter ProgressAction' {
			$parameter = (Get-Command Set-BapTenantDetail).Parameters['ProgressAction']
	...
	Describe "Testing parameterset __AllParameterSets" {
		<#
		__AllParameterSets -Id -Upn -TenantId
		__AllParameterSets -Id -Upn -TenantId -TenantName -ProgressAction
		#>
	}
