Describe "Get-FnOEnvironmentSecurityRole Unit Tests" -Tag "Unit" {
	BeforeAll {
	...
	Describe "Ensuring unchanged command signature" {
		It "should have the expected parameter sets" {
			(Get-Command Get-FnOEnvironmentSecurityRole).ParameterSets.Name | Should -Be '__AllParameterSets'
		}
		
		It 'Should have the expected parameter EnvironmentId' {
			$parameter = (Get-Command Get-FnOEnvironmentSecurityRole).Parameters['EnvironmentId']
		...
		It 'Should have the expected parameter Name' {
			$parameter = (Get-Command Get-FnOEnvironmentSecurityRole).Parameters['Name']
		...
		It 'Should have the expected parameter AsExcelOutput' {
			$parameter = (Get-Command Get-FnOEnvironmentSecurityRole).Parameters['AsExcelOutput']
