Describe "Get-PpacApplicationUser Unit Tests" -Tag "Unit" {
	BeforeAll {
	...
	Describe "Ensuring unchanged command signature" {
		It "should have the expected parameter sets" {
			(Get-Command Get-PpacApplicationUser).ParameterSets.Name | Should -Be '__AllParameterSets'
		}
		
		It 'Should have the expected parameter EnvironmentId' {
			$parameter = (Get-Command Get-PpacApplicationUser).Parameters['EnvironmentId']
		...
		It 'Should have the expected parameter User' {
			$parameter = (Get-Command Get-PpacApplicationUser).Parameters['User']
		...
		It 'Should have the expected parameter IncludePpacApplications' {
			$parameter = (Get-Command Get-PpacApplicationUser).Parameters['IncludePpacApplications']
		...
		It 'Should have the expected parameter AsExcelOutput' {
			$parameter = (Get-Command Get-PpacApplicationUser).Parameters['AsExcelOutput']
		...
		It 'Should have the expected parameter ProgressAction' {
			$parameter = (Get-Command Get-PpacApplicationUser).Parameters['ProgressAction']
	...
	Describe "Testing parameterset __AllParameterSets" {
		...
		__AllParameterSets -EnvironmentId -User -IncludePpacApplications -AsExcelOutput -ProgressAction
		#>
	}
