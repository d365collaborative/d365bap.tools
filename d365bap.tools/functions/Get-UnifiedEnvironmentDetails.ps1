...
#>
function Get-UnifiedEnvironmentDetails {
    ...
    process {
        ...
        $resCol = $filteredEnvs | ForEach-Object -Parallel {
            ...
            # Version details come from the Dataverse Web API action msprov_getfinopsapplicationdetails
            # Called as POST with an empty JSON body, same way Visual Studio requests it
        ...
        if ($AsExcelOutput) {
            $resCol | Export-Excel `
                -WorksheetName "Get-UnifiedEnvironmentDetails" `
                -NoNumberConversion PpacProvApp,PpacProvPlatform,ProvisioningAppVersion,ProvisioningPlatVersion
            return
        }
